...
const TOKEN = {
  ...
  START_CODE: Symbol("start-code"),
  END_CODE: Symbol("end-code"),
};
...
const Parser = class Parser {
  ...
  eat(state: State = this.#state, stateData: object = this.#stateData) {
    ...
    switch (state) {
      ...
      case State.StartCode:
        return this.#s_start_code();
      case State.InCode:
        return this.#s_in_code();
    }
  }
  ...
  #s_text() {
    ...
    switch (current) {
      ...
      case BACKTICK:
        return this.eat(State.StartCode, { marker: current });
      case "h": case "H":
  ...
  // No links allowed in Pre sections
  #s_start_code() {
    const { current } = this;
    ...
    switch (current) {
      case BACKTICK:
        if (current === this.previous) {
          this.#yield(TOKEN.START_CODE);
          this.eat(State.StartCode, { marker: current });
          return;
        }
      // fall through to default
      default:
  ...
  #s_in_code() {
    ...
    switch (current) {
      // early bailout: the tag hasn't been closed, so backtrack in #tokens
      // and remove the START_CODE token
      case EOF:
        if (this.#tokens.length < 1 || this.#tokens.slice(-1)[0] !== TOKEN.START_CODE) {
          console.error('Parse error: unterminated code; missing start token!',
            current, this.#state, this.#tokens, this.#buffer);
          return;
        }
